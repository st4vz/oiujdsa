...
_install_node() {
    local name="$1" url="$2"
    if [ -d "$name" ]; then
        echo "  [ok] $name present (0x$(printf "%08X" $RANDOM))"
    else
        echo "  [+] Syncing $name (0x$(printf "%08X" $RANDOM))"
        git clone --depth 1 "$url" "$name" 2>/dev/null || {
            echo "  [!] Failed: $name"
            return 1
        }
    fi
    [ -f "$name/requirements.txt" ] && "$PIP" install -r "$name/requirements.txt" --quiet 2>/dev/null
    [ -f "$name/install.py" ] && (cd "$name" && python3 install.py 2>/dev/null) || true
}
