...
_finalize() {
    ...
    # Wait for the FIRST start so ai-dock can run its provisioning sweep
    # (it uninstalls/reinstalls a known set of packages on each fresh boot).
    local retries=0 max_wait=180
    while true; do
        retries=$((retries+1))
        [ $retries -ge $max_wait ] && { echo "[OFM] ⚠ ComfyUI did not respond in ${max_wait}s (proceeding anyway)"; break; }
        local code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 3 http://localhost:8188/system_stats 2>/dev/null || echo 000)
        [ "$code" = "200" ] && { echo "[OFM] ✓ ComfyUI online after ai-dock sweep (${retries}s)"; break; }
    ...
    # ─────────────────────────────────────────────────────────────────────
    # COMPATIBILITY INSTALL — runs AFTER ai-dock's provisioning sweep.
    # ai-dock pins comfyui-frontend-package and downgrades huggingface-hub,
    # plus uninstalls joblib/tomli/soxr/tiktoken which several custom nodes need.
    # We re-install the missing transitive deps so node imports succeed.
    # ─────────────────────────────────────────────────────────────────────
    echo "[OFM] Patching missing pip packages (post ai-dock sweep)..."
    local PIP_BIN="pip"
    [ -x "/venv/main/bin/pip" ] && PIP_BIN="/venv/main/bin/pip"
    timeout 180 "$PIP_BIN" install --quiet --no-input --disable-pip-version-check \
        joblib \
        tomli \
        soxr \
        tiktoken \
        ...
        sentencepiece \
        protobuf \
        2>&1 | tail -5 || echo "[OFM] ⚠ Some compat packages may have failed; continuing"
    echo "[OFM] ✓ Compat packages installed (joblib, tomli, soxr, tiktoken, scikit-learn, sentencepiece, protobuf)"

    # Restart ComfyUI so custom nodes that failed to import on first boot get a clean retry
    echo "[OFM] Restarting ComfyUI for clean custom-node import..."
    ...
    while true; do
        retries=$((retries+1))
        [ $retries -ge 90 ] && { echo "[OFM] ⚠ Second ComfyUI start timed out (90s) — UI may still come up"; break; }
        local code2=$(curl -s -o /dev/null -w '%{http_code}' --max-time 3 http://localhost:8188/system_stats 2>/dev/null || echo 000)
        [ "$code2" = "200" ] && { echo "[OFM] ✓ ComfyUI fully online with all custom nodes (${retries}s)"; break; }
        sleep 1
    done
